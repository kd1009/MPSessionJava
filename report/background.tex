...
For the branching case similar rules apply, where the type $G::= p \rightarrow p' : k \lbrace l_{j}:G_{j} \rbrace _{j \in J} $ is projected to local type $G @ p ::= !{l_{j}: (G_{j} @ p)}$. The analogous applies for the projection of $G$ onto participant $p'$.
...
The structure of SJ Runtime, which is effectively a \LST{.jar} library file that every program written in SJ has to import is shown in Figure \ref{fig:sjruntime}. 
